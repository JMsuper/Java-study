...
```
위 코드의 `main`메소드는 `SubClass's method`를 출력한다. SuperClass형 reference 변수에 SubClass의 인스턴스가 대입되었다. superClass는 부모클래스 type의 변수이므로 SuperClass의 method를 호출할 것 같지만, 자식클래스의 메소드를 호출한다. 이러한 바인딩을 동적 바인딩이라고 말한다.
